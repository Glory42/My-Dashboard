...
export default function AddColumnModal({ workspaceId, onClose }: Props) {
  const [name, setName]   = useState('')
  const [color, setColor] = useState(COLORS[0])
  const qc = useQueryClient()

  const add = useMutation({
    mutationFn: () => api.post('/columns', { workspaceId, name: name.trim(), color }),
    onSuccess: () => {
    ...
    >
      <div className="bg-rp-surface border border-rp-hl-med rounded-lg p-5 w-full max-w-sm">
        <div className="flex items-center justify-between mb-4">
          <span className="font-mono text-xs font-semibold text-rp-text tracking-wide uppercase">add column</span>
